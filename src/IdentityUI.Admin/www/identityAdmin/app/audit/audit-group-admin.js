class AuditGroupAdmin {
    ...
    export() {
        ...
        fetch(`/api/GroupAdmin/${this.groupId}/Audit/Export?${new URLSearchParams(validParams).toString()}`)
            .then((resp) => {
                if (!resp.ok) {
...
class AuditTable {
    constructor(actionTypes, subjectTypes, statusAlert, groupId) {
        this.groupId = groupId;
        ...
        this.auditDetailsModal = new AuditDetailsModal([], this.groupId);
        this.auditTableFilters = new AuditTableFilters(this.groupId, subjectTypes, actionTypes, this.statusAlert, () => {
            this.reloadTable();
        });

        this.auditCommentModal = new AuditCommentModal(this.groupId);
...
class AuditCommentModal {
    ...
    addComment() {
        ...
        Api.post(`/api/GroupAdmin/${this.groupId}/Audit/AddComment/${this.id}`, this.getData())
            .done(() => {
                this.commentArea.value(null);
                this.getComments();
            })
    ...
    getComments() {
        ...
        Api.get(`/api/GroupAdmin/${this.groupId}/Audit/GetComments/${this.id}`)
            .done((data) => {
